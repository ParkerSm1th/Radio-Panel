...
$stmt->bindValue(':id', session_id());
 ...
 if ($count == 0 && $currentDJ != $userId && $permRole < 4) {
   ?>
   <script>
 ...
 if ($extra == false) {
  ...
  $('.coverSlot').click(function(ev) {
    ...
    $.ajax({
        type: 'POST',
        url: window.RadioPanelApp.apiBase + '/coverCurrent'
  ...
  $('.notificationDismiss').click(function(ev) {
      ...
      $.ajax({
          type: 'POST',
          url: window.RadioPanelApp.apiBase + '/deleteNotification',
