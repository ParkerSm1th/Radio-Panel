                ...
                if ($away == 0) {
                  ...
                  if ($count < $min) {
                    ...
                    ?>
                      <tr>
                        <td class="py-1">
                          <img src="<?php echo web_path('profilePictures/' . $row['avatarURL']); ?>" onerror="this.src='<?php echo web_path('assets/images/default.png'); ?>'" alt="image"> <span class="userLink" onclick="loadProfile(<?php echo $row['id'] ?>)"><?php echo $row['username'] ?></span>
                        </td>
                        <td>
                          <?php echo $count ?>
                        </td>
...
$(".warnUser").on("click", function () {
  ...
  $.ajax({
      type: 'GET',
      url: window.RadioPanelApp.apiBase + '/addPoint?min=1&user=' + elem.attr('data-id')
