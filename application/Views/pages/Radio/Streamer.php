<?php
  ...
  if ($bookedUser != '0') {
    ...
    $next_dj = "Unbooked";
    $next_dj_img = web_path('assets/images/square.png');
        ...
        <ul class="navbar-nav navbar-nav-right">
          <li class="nav-item dropdown d-none d-xl-inline-block">
            <a class="nav-link" id="UserDropdown" href="#" data-toggle="dropdown" aria-expanded="false">
              ...
              <img class="img-xs rounded-circle" src="<?php echo web_path('profilePictures/' . $_SESSION['loggedIn']['avatarURL']); ?>" onerror="this.src='<?php echo web_path('assets/images/default.png'); ?>'" alt="Profile image">
            </a>
          </li>
        </ul>
    ...
    <div class="row">
      <div class="col-3">
          ...
          <div class="s-box">
            ...
            <div class="s-box-main">
              <div class="nextDJ">
                <img src="<?php echo $next_dj_img?>" onerror="this.src='<?php echo web_path('assets/images/square.png'); ?>'"></img>
...
function updateStreamerStats(url) {
  return new Promise((resolve, reject) => {
		$("#likes").load(window.RadioPanelApp.apiBase + '/stats?specific=likes', function() {
		  resolve("Success!");
		});
    $("#time").load(window.RadioPanelApp.apiBase + '/stats?specific=time', function() {
		  resolve("Success!");
		});
...
$.ajax({
  type: 'GET',
  url: window.RadioPanelApp.apiBase + '/listenerCount?type=h'
...
function checkNew() {
  $.ajax({
      type: 'GET',
      url: window.RadioPanelApp.apiBase + '/newestRequest?streamer=1'
  }).done(function(response) {
    if (latest == null) {
      latest = response;
      return true;
...
function deleteRequest(elem) {
  ...
  $.ajax({
      type: 'POST',
      url: window.RadioPanelApp.apiBase + '/deleteRequest?id=' + $(elem).attr('data-id')
  }).done(function(response) {
...
$(".deleteRequest").on("click", function () {
  ...
  $.ajax({
      type: 'POST',
      url: window.RadioPanelApp.apiBase + '/deleteRequest?id=' + $(this).attr('data-id')
  }).done(function(response) {
    console.log(response);
    if (response == "deleted") {
      $(this).parent().parent().parent().fadeOut();
...
function reportRequest(elem) {
  ...
  $.ajax({
      type: 'POST',
      url: window.RadioPanelApp.apiBase + '/reportRequest?id=' + $(elem).attr('data-id')
  }).done(function(response) {
    console.log(response);
...
$(".reportRequest").on("click", function () {
  ...
  $.ajax({
      type: 'POST',
      url: window.RadioPanelApp.apiBase + '/reportRequest?id=' + $(this).attr('data-id')
  }).done(function(response) {
    console.log(response);
    if (response == "reported") {
      $(this).parent().parent().parent().fadeOut();
...
$(form).submit(function(event) {
    ...
    $.ajax({
        type: 'POST',
        url: window.RadioPanelApp.apiBase + '/setDJSays',
        data: formData
    }).done(function(response) {
      console.log(response);
      if (response == 'updated') {
