     ...
     <div class="row">
      ...
      foreach($stmt as $row) {
          ...
          <script>
            $.ajax({
                type: 'POST',
                url: `${window.RadioPanelApp.apiBase}/manageAssignments?action=get&user=<?php echo $userDetails['id']?>`
            }).done(function(response) {
              var prePop = JSON.parse(response);
              $("#autoc<?php echo $userDetails['id']?>").tokenInput("' + window.RadioPanelApp.apiBase + '/findReviewUser", {
                theme: "mac",
                ...
                onAdd: function (item) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: `${window.RadioPanelApp.apiBase}/manageAssignments?action=add&user=${admin}&assigned=${assigned}`
                    }).done(function(response) {
                ...
                onDelete: function (item) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: `${window.RadioPanelApp.apiBase}/manageAssignments?action=del&user=${admin}&assigned=${assigned}`
                    }).done(function(response) {
