 ...
 ?>
                ...
                <?php
                  ...
                  foreach($stmt as $row) {
                    ...
                    ?>
                      <tr>
                        <td class="py-1">
                          <img src="<?php echo web_path('profilePictures/' . $row['avatarURL']); ?>" onerror="this.src='<?php echo web_path('assets/images/square.png'); ?>'" alt="image"><span onclick="loadProfile('<?php echo $row['id'] ?>')" class="userLink"> <?php echo $row['username'] ?></span>
                        </td>
                        <td>
                          <?php
                            if ($row['radio'] == '1') {
    ...
    <?php
     ...
     $('.passUser').click(function(ev) {
         ...
         $.ajax({
             type: 'POST',
             url: window.RadioPanelApp.apiBase + '/passUser',
     ...
     $('.failUser').click(function(ev) {
         ...
         $.ajax({
             type: 'POST',
             url: window.RadioPanelApp.apiBase + '/failUser',
             data: {id: thing.dataset.uid}
         }).done(function(response) {
           console.log(response);
           if (response == 'passed') {
