...
$bookedUser = $row ? $row['booked'] : '0';
$now_id = $row ? $row['id'] : 0;
$type = $row ? $row['booked_type'] : '0';
if ($bookedUser != '0') {
  $stmt = $conn->prepare("SELECT * FROM users WHERE id = '$bookedUser'");
  $stmt->execute();
  $row = $stmt->fetch(PDO::FETCH_ASSOC);
  $now_dj = $row['username'];
  $now_dj_img = web_path('profilePictures/' . $row['avatarURL']);
  if ($type == '1') {
...
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$bookedUser = $row ? $row['booked'] : '0';
$next_id = $row ? $row['id'] : 0;
if ($bookedUser != '0') {
  $stmt = $conn->prepare("SELECT * FROM users WHERE id = '$bookedUser'");
  $stmt->execute();
  $row = $stmt->fetch(PDO::FETCH_ASSOC);
  $next_dj = $row['username'];
  $next_dj_img = web_path('profilePictures/' . $row['avatarURL']);
...
$row = $stmt->fetch(PDO::FETCH_ASSOC);
$bookedUser = $row ? $row['booked'] : '0';
$later_id = $row ? $row['id'] : 0;
if ($bookedUser != '0') {
  $stmt = $conn->prepare("SELECT * FROM users WHERE id = '$bookedUser'");
  $stmt->execute();
  $row = $stmt->fetch(PDO::FETCH_ASSOC);
  $later_dj = $row['username'];
  $later_dj_img = web_path('profilePictures/' . $row['avatarURL']);
...
<div class="row">
		...
		<div class="card" style="border-left: 2px solid #1b7305;">
			<div class="card-body">
				<div class="d-flex flex-row">
					<img id="img-<?php echo $now_id?>" src="<?php echo $now_dj_img ?>" onerror="this.src='<?php echo web_path('assets/images/default.png'); ?>'" class="mini-tt img-lg rounded" alt="profile image">
					<div class="mini-tt">
            <div id="now">
              <?php 
              if ($now_booked) {
	...
	<div class="col-md-4 grid-margin stretch-card">
		<div class="card" style="border-left: 2px solid #a9a40b;">
			<div class="card-body">
				<div class="d-flex flex-row">
          <img id="img-<?php echo $next_id?>" src="<?php echo $next_dj_img ?>" onerror="this.src='<?php echo web_path('assets/images/default.png'); ?>'" class="mini-tt img-lg rounded" alt="profile image">
          <div class="mini-tt">
            <div id="next">
            <?php 
              if ($next_booked) {
	...
	<div class="col-md-4 grid-margin stretch-card">
		<div class="card" style="border-left: 2px solid #a20606;">
			<div class="card-body">
				<div class="d-flex flex-row">
          <img id="img-<?php echo $later_id?>" src="<?php echo $later_dj_img ?>" onerror="this.src='<?php echo web_path('assets/images/default.png'); ?>'" class="mini-tt img-lg rounded" alt="profile image">
          <div class="mini-tt">
          <div id="later">
            <?php 
              if ($later_booked) {
...
function updateStats(url) {
  return new Promise((resolve, reject) => {
		$("#listeners").load(window.RadioPanelApp.apiBase + '/stats?specific=listeners', function() {
		  resolve("Success!");
    });
    $("#list").load(window.RadioPanelApp.apiBase + '/stats?specific=listeners', function() {
		  resolve("Success!");
		});
    $("#listenersPeak").load(window.RadioPanelApp.apiBase + '/stats?specific=listenersPeak', function() {
		  resolve("Success!");
    });
    $.ajax({
        type: 'GET',
...
$('.bookButton').click(function () {
  ...
  $.ajax({
      type: 'POST',
      url: window.RadioPanelApp.apiBase + '/bookSlot',
      data: {id: id}
  }).done(function(response) {
    console.log(response);
    if (response == 'booked') {
      $("#img-" + id).attr('src', <?php echo json_encode(web_path('profilePictures/' . $_SESSION['loggedIn']['avatarURL'])); ?>);
      ...
      setTimeout(function () { object.html('<p>Cover</p>'); }, 2000);
    }
  }).fail(function (response) {
    object.html('<p><i class="fas fa-times" style="color: #fff; font-size: 13px;"></i> Failed</p>');
  });
...
$.ajax({
  type: 'GET',
  url: window.RadioPanelApp.apiBase + '/listenerCount?type=d'
}).done(function (response) {
...
$.ajax({
  type: 'GET',
  url: window.RadioPanelApp.apiBase + '/listenerCount?type=w'
}).done(function (response) {
